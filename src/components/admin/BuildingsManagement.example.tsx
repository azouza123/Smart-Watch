// EXEMPLE : Comment connecter les boutons des bâtiments au backend
// Remplacez le contenu de BuildingsManagement.tsx par cette version
...
export function BuildingsManagement() {
  ...
  const loadBuildings = async () => {
    try {
      ...
      const adaptedBuildings: Building[] = data.map((b: any) => ({
        id: b.id || b.idbatiment,
        name: b.nom || b.nombat,
        address: b.adresse,
        objectif: b.objectif || '',
        zones: 0, // À calculer ou ajouter dans le backend
        units: 0, // À calculer ou ajouter dans le backend
        sensors: b.nbreDeCapteurs || 0,
        status: 'active'
      }));
  ...
  const handleAddBuilding = async (e: React.FormEvent) => {
    ...
    try {
      await adminApi.createBuilding({
        nom: newBuilding.name,
        adresse: newBuilding.address,
        objectif: newBuilding.objectif,
        nbreDeCapteurs: newBuilding.sensors,
        // type sera déterminé par le backend selon la classe
      });
      await loadBuildings(); // Recharger la liste
      setNewBuilding({ name: '', address: '', objectif: '', zones: 0, units: 0, sensors: 0 });
  ...
  const handleUpdateBuilding = async (e: React.FormEvent) => {
    ...
    if (editingBuilding) {
      try {
        await adminApi.updateBuilding(editingBuilding.id, {
          nom: editingBuilding.name,
          adresse: editingBuilding.address,
          objectif: editingBuilding.objectif,
          nbreDeCapteurs: editingBuilding.sensors,
        });
        await loadBuildings(); // Recharger la liste
  ...
  // Gestion des zones (pour l'instant en local, à implémenter dans le backend si nécessaire)
  ...
  // Charger les capteurs d'un bâtiment
  const loadSensorsForBuilding = async (buildingId: number) => {
    try {
      const sensors = await adminApi.getSensorsByBuilding(buildingId);
      // Mettre à jour le nombre de capteurs dans le bâtiment
      setBuildings(prev => prev.map(b => 
        b.id === buildingId ? { ...b, sensors: sensors.length } : b
      ));
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="p-6">Chargement des bâtiments...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleAddBuilding} className="space-y-6">
          ...
          <InputField
            label="Objectif"
            value={newBuilding.objectif}
            onChange={(e) => setNewBuilding(prev => ({...prev, objectif: e.target.value}))}
            placeholder="Objectif d'économie d'eau"
          />
      ...
      >
        <form onSubmit={handleUpdateBuilding} className="space-y-6">
          ...
          <InputField
            label="Objectif"
            value={editingBuilding?.objectif || ''}
            onChange={(e) => setEditingBuilding(prev => prev ? {...prev, objectif: e.target.value} : null)}
            placeholder="Objectif d'économie d'eau"
          />
      ...
      {/* Modales pour les zones (identique à l'original) */}
      <Modal 
            ...
            {buildings.map((building) => (
              <div 
                ...
                onClick={() => {
                  ...
                  loadSensorsForBuilding(building.id);
                }}
              >
